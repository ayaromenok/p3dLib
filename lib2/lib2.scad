...
module yCyl(r=0, szz=1, px=0, py=0, pz=0, rx=0, ry=0, rz=0, sx=1, sy=1, sz=1, mx=0, my=0, mz=0, rb=1, rt=1, clr = "grey", fa=($preview ? 12:0.1), fs=($preview ? 2:0.4), isFn=false, $fn = 0 ) {
    ...
    color(clr)    
    if ($fn==0){        
        cylinder(szz, rb, rt,  $fa=fa, $fs=fs);            
    } else {        
        cylinder(szz, rb, rt);            
    }//isFn
}//yCyl
